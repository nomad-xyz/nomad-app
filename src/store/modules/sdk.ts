...
const actions = <ActionTree<SDKState, RootState>>{
  ...
  async processTx({ dispatch }, tx: any) {
    ...
    // register signer
    const domainName = message.context.resolveDomainName(message.destination)!;
    await dispatch("switchNetwork", domainName);

    // get proof
    const res = await fetch(
      `${s3URL}${origin}_${message.leafIndex.toString()}`
    );
    const data = (await res.json()) as any;
    console.log("proof: ", data);
    // get replica contract
    const replica = nomad
      .mustGetCore(message.destination)
      .getReplica(message.origin);

    console.log(replica!.signer);
